...
export const handler = async (
  ...
  const format = (ctx.url.searchParams.get("format") as "png" | "jpeg" | "webp" | undefined) || "png"
...
const resizeImage = async (input: string, width: number, height: number, format?: "png" | "jpeg" | "webp") => {
  let image = new Image()

  if (input.startsWith("http"))
    image = await Image.load(input)
