...
export async function handler(req: Request, ctx: FreshContext) {
  ...
  if (req.method === "OPTIONS") {
    ...
    headers.set("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Requested-With, X-Redirected, Accept, Cache-Control, Last-Event-ID");
  ...
  if (session) {
    ...
    if (sessionData) {
      ...
      const currentUserIndex = sessionData.accounts.findIndex((acc: any) => acc.id === sessionData.activeAccount)
      const currentUser = sessionData.accounts[currentUserIndex]

      if (currentUser)
      if (!currentUser.accessToken || (currentUser.tokenUpdatedAt && now - currentUser.tokenUpdatedAt > tokenExpiresIn)){
  ...
  //   } else {
  //     console.log("No cached session found, querying Supabase...");
