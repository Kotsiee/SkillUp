...
export function DualSlider(props: JSX.HTMLAttributes<HTMLInputElement> & DualSliderProps) {
    const minVal = useSignal((props.startMinVal != 'min' ? props.startMinVal : props.minVal) || props.minVal);
    const maxVal = useSignal((props.startMaxVal != 'max' ? props.startMaxVal : props.maxVal) ||  props.maxVal);
        ...
        <div class={props.class}>
            ...
            <div className={`${props.className} DualSlider`}>
                ...
                <div className={`${props.className} DualSlider-slider-container`}>
                    <div ref={range} className={`${props.className} DualSlider-slider-range`}
                        style={{
                            left: `${getPercentage(props.minVal, props.maxVal, Math.min(minVal.value, maxVal.value)) }%`,
                            width: `${getPercentage(props.minVal, props.maxVal, Math.max(minVal.value, maxVal.value) - Math.min(minVal.value, maxVal.value) )}%`
                        }}
                    />
                </div>
            </div>
            ...
            <div className={`${props.className} DualSlider-Inputs`}>
                ...
                onChange={e => {
                    minVal.value = roundToStep( parseFloat(e.currentTarget.value), (props.step as number)? props.step as number : 1 )
                }}
                />
