  ...
  return (
    <div class="preview">
      ...
      <div class="preview-info">
        <div class="obj-info">
          <h5 class="title">{fileRef.fileRef.publicName ?? file.publicName}</h5>
          <p class="obj-type">{file.meta?.application || file.fileType || file.mimeType}</p>
          <p class="sent-at">{createdAtDate.setLocale('en-GB').toFormat("DDDD' - 'HH':'mm")}</p>
        </div>
        <hr />
        ...
        <ul>
          <li class="open">
            <a
              ...
              f-partial={`${globalThis.location.pathname
                .split('/attachments')
                .join('/partials/attachments')}/${fileRef.fileRef.id}`}
            >
              Open
            </a>
          </li>
