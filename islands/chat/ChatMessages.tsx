...
const ChatSection = (
  ...
  const handleSubscription = (
    ...
    switch (payload.eventType) {
      case "INSERT":
        setMessages((
          prevMessages,
        ) => [...prevMessages, toMessage(payload.new as Messages, c)]);
        break;
      case "UPDATE":
        setMessages((prevMessages) =>
          prevMessages.map((message) =>
            message.id === (payload.new as Messages).id
              ? { ...message, content: (payload.new as Messages).content }
              : message
          )
        );
        break;
      case "DELETE":
        setMessages((prevMessages) =>
          prevMessages.filter((message) =>
            message.id !== (payload.old as Messages).id
          )
        );
        break;
    }
  ...
  useEffect(() => {
    let channel: any;
    async function fetchMessages() {
      const res1 = await fetch(`/api/chats/${props.chat.id}/messages`, {
        method: "GET",
      });
  ...
  const getChatInfo = (type: "photo" | "name") => {
    ...
    const otherUser = props.chat.users!.find((u) => u.user?.id != props.user.id)
      ?.user!;

    switch (type) {
      case "photo":
        return props.chat.photo != null
          ? props.chat.photo.url
  ...
  return (
    <div>
      ...
      <div class="chat-messages-area">
        {props.p.type === "messages"
          ? (
            <div class="top">
              <div>
                <img class="chat-messages-photo" src={getChatInfo("photo")} />
              </div>
              <div>
                <p>&#9432; Chat Info</p>
              </div>
          ...
          {messages?.map((msg, index) => {
            return (
              <ChatMessage
                ...
                prevUID={messages[index - 1]?.user?.user?.id}
              />
            );
          })}
        </ul>
...
const ChatMessage = (
  props: { msg: Messages; userId: string; prevUID?: string },
) => {
        ...
        <div class="message">
          ...
          <div class="content">
            ...
            <p
              ...
              ref={seeMore}
              onClick={() => {
                seeMoreVal.value = !seeMoreVal.value;
              }}
            >
