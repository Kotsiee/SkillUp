...
export async function fetchUsers(): Promise<User[] | null> {
  const { data, error } = await getSupabaseClient().schema('users').from('users').select('*');
  ...
  const users: User[] = await Promise.all(
    data.map(async d => {
      ...
      const ppSmall = await fetchFileByPath(newUser.id, `profile/avatar`, 'ppSmall.webp');
      const ppMed = await fetchFileByPath(newUser.id, `profile/avatar`, 'ppMed.webp');
      const ppLarge = await fetchFileByPath(newUser.id, `profile/avatar`, 'ppLarge.webp');
