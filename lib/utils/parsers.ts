export interface ProjectFilter{
  ...
  export function joinProjectFilter(filter: ProjectFilter | null): string {
    ...
    if (filter.minContributors !== undefined) strArr.push(`minContributors=${filter.minContributors}`);
    if (filter.maxContributors !== undefined) strArr.push(`maxContributors=${filter.maxContributors}`);
